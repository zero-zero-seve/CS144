...
class NetworkInterface {
   public:
    ...
    // addresses
    NetworkInterface(std::string_view name, std::shared_ptr<OutputPort> port,
    ...
    // address). Will need to use [ARP](\ref rfc::rfc826) to look up the Ethernet destination address for the
    // next hop. Sending is accomplished by calling `transmit()` (a member variable) on the frame.
   ...
   private:
    // arp映射表
    std::unordered_map<uint32_t, std::pair<EthernetAddress, uint64_t>> arp_map_;
    // 发出去但是未得到响应的arp消息（只需要记录目标ip地址即可）,同时记录时间，超时后重新发送
    std::unordered_map<uint32_t, uint64_t> wait_arp_msgs;
    // 缓存队列，缓存由于目标mac地址未知暂时没有发送的 ip数据报
    std::list<std::pair<Address, IPv4Datagram>> cache_queue_;
    // 时间变量
    uint64_t time_;
