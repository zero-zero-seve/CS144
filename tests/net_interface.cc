    ...
    try {
        {
            ...
            test.execute(ReceiveFrame{make_frame(target_eth, local_eth,
            ...
            // test.execute(ExpectFrame{make_frame(
            //     local_eth, ETHERNET_BROADCAST, EthernetHeader::TYPE_ARP,
            //     serialize(make_arp(ARPMessage::OPCODE_REQUEST, local_eth, "4.3.2.1", {},
            //     "192.168.0.1")))});
            ...
            test.execute(ReceiveFrame(
                make_frame(target_eth, local_eth, EthernetHeader::TYPE_IPv4, serialize(reply_datagram)),
                reply_datagram));
            test.execute(ExpectNoFrame{});
...

        {
            ...
            test.execute(ReceiveFrame{make_frame(
                remote_eth, ETHERNET_BROADCAST, EthernetHeader::TYPE_ARP,
            ...
            test.execute(ExpectFrame{make_frame(local_eth, remote_eth, EthernetHeader::TYPE_ARP,
                                                serialize(make_arp(ARPMessage::OPCODE_REPLY, local_eth,
                                                                   "5.5.5.5", remote_eth, "10.0.1.1")))});
            test.execute(ExpectNoFrame{});
        }

        {
            ...
            test.execute(ExpectFrame{
                make_frame(local_eth, remote_eth, EthernetHeader::TYPE_IPv4, serialize(datagram))});
            test.execute(ExpectNoFrame{});
        }

        {
            const EthernetAddress local_eth = random_private_ethernet_address();
            NetworkInterfaceTestHarness test{"pending mappings last five seconds", local_eth,
                                             Address("1.2.3.4", 0)};
        ...
        {
            const EthernetAddress local_eth = random_private_ethernet_address();
            NetworkInterfaceTestHarness test{"active mappings last 30 seconds", local_eth,
                                             Address("4.3.2.1", 0)};
            ...
            test.execute(ExpectFrame{
                make_frame(local_eth, target_eth, EthernetHeader::TYPE_IPv4, serialize(datagram2))});
            test.execute(ExpectNoFrame{});
            ...
            test.execute(ExpectFrame{
                make_frame(local_eth, target_eth, EthernetHeader::TYPE_IPv4, serialize(datagram3))});
            test.execute(ExpectNoFrame{});
            ...
            test.execute(ReceiveFrame{make_frame(new_target_eth, local_eth, EthernetHeader::TYPE_ARP,
                                                 serialize(make_arp(ARPMessage::OPCODE_REPLY, new_target_eth,
                                                                    "192.168.0.1", local_eth, "4.3.2.1")))});
            test.execute(ExpectFrame{
...

        {
            ...
            NetworkInterfaceTestHarness test{"different ARP mappings are independent", local_eth,
                                             Address("10.0.0.1", 0)};

            // first ARP mapping
            test.execute(ReceiveFrame{make_frame(
                remote_eth1, ETHERNET_BROADCAST, EthernetHeader::TYPE_ARP,
                serialize(make_arp(ARPMessage::OPCODE_REQUEST, remote_eth1, "10.0.0.5", {}, "10.0.0.1")))});
            test.execute(ExpectFrame{make_frame(local_eth, remote_eth1, EthernetHeader::TYPE_ARP,
                                                serialize(make_arp(ARPMessage::OPCODE_REPLY, local_eth,
                                                                   "10.0.0.1", remote_eth1, "10.0.0.5")))});
            test.execute(ExpectNoFrame{});
            ...
            // second ARP mapping
            test.execute(ReceiveFrame{make_frame(
                remote_eth2, ETHERNET_BROADCAST, EthernetHeader::TYPE_ARP,
                serialize(make_arp(ARPMessage::OPCODE_REQUEST, remote_eth2, "10.0.0.19", {}, "10.0.0.1")))});
            test.execute(ExpectFrame{make_frame(local_eth, remote_eth2, EthernetHeader::TYPE_ARP,
                                                serialize(make_arp(ARPMessage::OPCODE_REPLY, local_eth,
                                                                   "10.0.0.1", remote_eth2, "10.0.0.19")))});
            test.execute(ExpectNoFrame{});
            ...
            test.execute(ExpectFrame{
                make_frame(local_eth, remote_eth1, EthernetHeader::TYPE_IPv4, serialize(datagram))});
            test.execute(ExpectFrame{
                make_frame(local_eth, remote_eth2, EthernetHeader::TYPE_IPv4, serialize(datagram2))});
            test.execute(ExpectNoFrame{});
            ...
            test.execute(ExpectFrame{
                make_frame(local_eth, remote_eth2, EthernetHeader::TYPE_IPv4, serialize(datagram3))});
            test.execute(ExpectNoFrame{});
        ...
        // test credit: Matthew Harvill
        {
            ...
            NetworkInterfaceTestHarness test{
                "pending datagrams sent to correct dst when ARP replies received out-of-order", local_eth,
                Address("4.3.2.1", 0)};
            ...
            test.execute(ReceiveFrame{make_frame(target_eth2, local_eth,
                                                 EthernetHeader::TYPE_ARP,  // NOLINTNEXTLINE(*-suspicious-*)
                                                 serialize(make_arp(ARPMessage::OPCODE_REPLY, target_eth2,
                                                                    "192.168.0.2", local_eth, "4.3.2.1")))});
            test.execute(ExpectFrame{
                make_frame(local_eth, target_eth2, EthernetHeader::TYPE_IPv4, serialize(datagram2))});
            test.execute(ExpectNoFrame{});

            // Receive ARP reply from target_eth1 next, make sure we send datagram1
            test.execute(ReceiveFrame{make_frame(target_eth1, local_eth,
                                                 EthernetHeader::TYPE_ARP,  // NOLINTNEXTLINE(*-suspicious-*)
                                                 serialize(make_arp(ARPMessage::OPCODE_REPLY, target_eth1,
                                                                    "192.168.0.1", local_eth, "4.3.2.1")))});
            test.execute(ExpectFrame{
                make_frame(local_eth, target_eth1, EthernetHeader::TYPE_IPv4, serialize(datagram1))});
            test.execute(ExpectNoFrame{});
        }
        ...
        // test credit: Matthew Harvill
        {
            ...
            NetworkInterfaceTestHarness test{"multiple pending datagrams sent when ARP reply received",
                                             local_eth, Address("4.3.2.1", 0)};

            const auto datagram1 = make_datagram("5.6.7.8", "13.12.11.10");
            const auto datagram2 = make_datagram("5.6.7.8", "13.12.11.11");
            const auto datagram3 = make_datagram("5.6.7.8", "13.12.11.12");

            // Try to send all datagrams, but we don't know target_eth yet, so should send ARP request to get
            // it
            test.execute(SendDatagram{datagram1, Address("192.168.0.1", 0)});
            test.execute(SendDatagram{datagram2, Address("192.168.0.1", 0)});
            test.execute(SendDatagram{datagram3, Address("192.168.0.1", 0)});
            ...
            test.execute(ReceiveFrame{make_frame(target_eth, local_eth,
                                                 ...
                                                 serialize(make_arp(ARPMessage::OPCODE_REPLY, target_eth,
                                                                    "192.168.0.1", local_eth, "4.3.2.1")))});
            test.execute(ExpectFrame{
                make_frame(local_eth, target_eth, EthernetHeader::TYPE_IPv4, serialize(datagram1))});
            test.execute(ExpectFrame{
                make_frame(local_eth, target_eth, EthernetHeader::TYPE_IPv4, serialize(datagram2))});
            test.execute(ExpectFrame{
                make_frame(local_eth, target_eth, EthernetHeader::TYPE_IPv4, serialize(datagram3))});
        ...
        // test credit: Tanmay Garg
        {
            ...
            NetworkInterfaceTestHarness test{"update timestamp for ARP mapping if in map already", local_eth,
                                             Address("100.5.100.5", 0)};

            // receive ARP request
            test.execute(ReceiveFrame{make_frame(remote_eth, ETHERNET_BROADCAST, EthernetHeader::TYPE_ARP,
                                                 serialize(make_arp(ARPMessage::OPCODE_REQUEST, remote_eth,
                                                                    "144.144.144.144", {}, "100.5.100.5")))});

            // send ARP reply which does not reach destination
            test.execute(
                ExpectFrame{make_frame(local_eth, remote_eth, EthernetHeader::TYPE_ARP,
                                       serialize(make_arp(ARPMessage::OPCODE_REPLY, local_eth, "100.5.100.5",
                                                          remote_eth, "144.144.144.144")))});
            test.execute(ExpectNoFrame{});
            ...
            test.execute(ExpectFrame{
                make_frame(local_eth, remote_eth, EthernetHeader::TYPE_IPv4, serialize(datagram1))});
            test.execute(ExpectNoFrame{});
        ...
        {
            const EthernetAddress local_eth = random_private_ethernet_address();
            NetworkInterfaceTestHarness test{"Two datagrams sent before ARP response", local_eth,
                                             Address("4.3.2.1", 0)};
        ...
        {
            const EthernetAddress local_eth = random_private_ethernet_address();
            NetworkInterfaceTestHarness test{"Pending datagrams dropped when pending request expires",
                                             local_eth, Address("4.3.2.1", 0)};
        ...
        {
            const EthernetAddress local_eth = random_private_ethernet_address();
            NetworkInterfaceTestHarness test{"ARP replies trigger correct Ethernet frame", local_eth,
                                             Address("5.5.5.5", 0)};
            ...
            for (int i = 0; i < 50; i++) {
                ...
                test.execute(
                    SendDatagram{datagram_2, Address("192.168.0." + to_string(dst_suffix_3 + 1), 0)});
                test.execute(ExpectFrame{
                    make_frame(local_eth, ETHERNET_BROADCAST, EthernetHeader::TYPE_ARP,
                               serialize(make_arp(ARPMessage::OPCODE_REQUEST, local_eth, "5.5.5.5", {},
                ...
                // call recv_frame once, expecting only one ARP Reply followed by transmission of the correct
                // Ethernet Frame
                const EthernetAddress target_eth = random_private_ethernet_address();
                test.execute(ReceiveFrame{make_frame(
                    target_eth, local_eth,
                    EthernetHeader::TYPE_ARP,  // NOLINTNEXTLINE(*-suspicious-*)
                    serialize(make_arp(ARPMessage::OPCODE_REPLY, target_eth,
                                       "192.168.0." + to_string(dst_suffix_3 + 1), local_eth, "5.5.5.5")))});
                test.execute(ExpectFrame{
                    make_frame(local_eth, target_eth, EthernetHeader::TYPE_IPv4, serialize(datagram_2))});
                test.execute(ExpectNoFrame{});
